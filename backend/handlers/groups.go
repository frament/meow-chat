...
func (h *Handler) GetGroupMessages(c *fiber.Ctx) error {
	...
	rows, err := database.DB.Query(`
		SELECT m.id, m.from_user_id, COALESCE(m.msg_type, 'text'), m.content, m.created_at, u.username
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.FromUserID, &m.Type, &m.Content, &m.CreatedAt, &m.FromUser); err != nil {
			continue
		}
...
func (h *Handler) SendGroupMessage(c *fiber.Ctx) error {
	...
	result, err := tx.Exec(
		"INSERT INTO group_messages (group_chat_id, from_user_id, content, msg_type) VALUES (?, ?, ?, ?)",
		groupID, fromUserID, content, msgType,
	)
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "Failed to send message"})
	}
