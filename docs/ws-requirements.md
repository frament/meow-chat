...
- [ ] **T6**: Сообщение, отправленное из вкладки A, появляется во вкладке B (через WS).
- [ ] **T7**: При race между `getMessages` и `sendMessage` сообщение не теряется.
- [ ] **T8**: После reconnect все подписки продолжают работать (нет zombie-обработчиков).
