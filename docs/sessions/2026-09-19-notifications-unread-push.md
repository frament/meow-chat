...
## Дополнение (2026-09-24): блокировка отправки без ключа, фикс консольной ошибки

### Блокировка незашифрованной отправки
Живой прогон показал: если у собеседника нет публичного E2EE-ключа, отправка молча уходила в plaintext. Теперь `chat.ts:sendMessage` при `encryptable` (text/image с текстом) делает `crypto.init()` и, если `encrypt`/`encryptGroupMessage` вернули null, **не отправляет** и показывает инлайн-ошибку («у собеседника нет ключа шифрования» / «нет ключа группового чата»). Стикеры/GIF/опросы (структурные) не блокируются.

### Консольная ошибка «ERROR [object Object]»
Причина: не-админские клиенты дёргали админский `GET /api/admin/settings/giphy-key` (ради `has_key`) → 403 → необработанная ошибка. Добавлен `GET /api/giphy/status` (auth) → `{has_key}`; `chat.ts` использует его + error-handler. Плюс кастомный `ErrorHandler` в `app.config.ts` логирует сообщение/стек вместо `[object Object]`.

### E2E на проде (alice99 ↔ bob99)
- Дата-разделители «11 Июля»/«Сегодня», «НОВЫЕ СООБЩЕНИЯ» — ок.
- Honest E2EE: msg 239 в БД `content=0`, `encrypted_content=88`, `iv=16`; bob99 расшифровал.
- Push-логи в админке (вкладка Push): `subscribe`, `unsubscribe`, `rotate`, `no_subscription`, `permission_denied` — видны; 3 подписки, сводка.
- Версия в настройках — `v1.1.1-15-g186d751` (актуальна на момент прогона).

Сервисные учётки: сброшены пароли alice99/bob99/admin, тестовые сообщения 238/239 удалены.
