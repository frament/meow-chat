    ...
    <div class="min-h-screen" style="background:var(--bg-body);">
              ...
              @if (api.chatHeaderInfo(); as header) {
                <div class="sm:hidden flex items-center gap-2">
                  ...
                  @if (header.type === 'user') {
                    ...
                    <h3 class="font-medium text-sm" style="color:var(--text-primary);">{{ header.name }}</h3>
                    <button (click)="api.groupInfoRequest$.next(header.id)" aria-label="Управление группой"
                      style="width:32px;height:32px;display:flex;align-items:center;justify-content:center;border:none;border-radius:var(--radius-sm);background:transparent;color:var(--text-tertiary);cursor:pointer;">
      ...
      <nav class="bottom-nav fixed bottom-0 left-0 right-0 sm:hidden z-50" style="padding-bottom:env(safe-area-inset-bottom,0px);">
        <div class="flex items-center justify-around h-14">
          ...
          <a routerLink="/chat" routerLinkActive="active-link" class="nav-link bottom-nav-link relative">
            <span class="relative">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
              @if (api.totalUnread()) {
                <span class="badge-nav-sm">{{ api.totalUnread() }}</span>
              }
            </span>
